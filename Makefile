...
isort_check: ## check that isort has been run
	isort --check-only sanctions/

isort: ## run isort to sort imports in all Python files
	isort --atomic sanctions/
...
open-devstack: ## open a shell on the server started by start-devstack
	docker exec -it sanctions /edx/app/sanctions/devstack.sh open

pkg-devstack: ## build the sanctions image from the latest configuration and code
	docker build -t sanctions:latest -f docker/build/sanctions/Dockerfile git://github.com/openedx/configuration

detect_changed_source_translations: ## check if translation files are up-to-date
	cd sanctions && i18n_tool changed

validate_translations: fake_translations detect_changed_source_translations ## install fake translations and check if translation files are up-to-date

docker_build:
	docker build . -f Dockerfile -t openedx/sanctions
...
app-shell: # Run the app shell as root
	docker exec -u 0 -it sanctions.app bash

db-shell: # Run the app shell as root, enter the app's database
	docker exec -u 0 -it sanctions.db mysql -u root sanctions
...
%-attach:
	docker attach sanctions.$*

github_docker_build:
	docker build . -f Dockerfile --target app -t openedx/sanctions

github_docker_tag: github_docker_build
	docker tag openedx/sanctions openedx/sanctions:${GITHUB_SHA}

github_docker_auth:
	echo "$$DOCKERHUB_PASSWORD" | docker login -u "$$DOCKERHUB_USERNAME" --password-stdin

github_docker_push: github_docker_tag github_docker_auth ## push to docker hub
	docker push 'openedx/sanctions:latest'
	docker push "openedx/sanctions:${GITHUB_SHA}"
